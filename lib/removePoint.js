...
export const removePoint1 = (match) => {
    ...
    if (current_set > 2 && (match.matchFormat !== 'best-of-3')) {
    ...
    if (current_set === 1) {
        ...
        if (isTiebreak) {
            var active = set_active(match.tiebreak1)
            if (active) {
            ...
            var active = set_active(match.game)
            if (active) {
...
export const removePoint2 = (match) => {
    var match_status = sets_status(match.set1, match.set2, match.set3, match.supertiebreak)
    var current_set = set_count(match_status)
